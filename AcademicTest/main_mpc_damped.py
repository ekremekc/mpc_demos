...
a_form = -c**2 * inner(grad(u), grad(v))*dx

e_form =  -c**2 * K_r/d**2 * (inner(u, v)*ds(inner_tag)-inner(u, v)*ds(outer_tag))
# e_form =  -c**2 * K_r/d**2 * inner(u, v)*ds(inner_tag) \
#           +c**2 * K_r/d**2 * inner(u, v)*ds(outer_tag)
...
with XDMFFile(MPI.COMM_WORLD, "results/eigenvector_damped.xdmf", "w", encoding=XDMFFile.Encoding.HDF5) as xdmf:
